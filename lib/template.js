...
<!-- login 로그인 -->
  ...
  modify: (sldodata) => {
    ...
    <form class="form my-2 my-lg-0" action="/community/write_process" method="post">
      <div class="my-3">
        <h4><input type="text" name="subject" value="${sldodata[0].subject}" style="width: 540px;" placeholder="제목"></h4>
        <b>${sldodata[0].name}</b>
      </div>
      <div class="mt-3 text-center">
        <textarea name="description" style="width:100%; height: 480px;">${sldodata[0].description}</textarea>
      </div>
      <div>
        <span>
        ...
        <span>
          <input type="button" value="취소" onclick="location.replace('/community/board_detail?id=${sldodata[0].id}');" class="btn btn-danger">
        </span>
      </div>
    </form>
  `;
  ...
  fbdetail: (sldofboard) => {
    ...
    <div>
      ...
      <div class="my-5">
        ...
        <span class="mr-2">
          <input type="button" value="수정" onclick="location.replace('/community/modify?id=${sldofboard[0].id}');" class="btn btn-warning">
        </span>
        <span class="ml-2">
          <input type="button" value="목록으로" onclick="location.replace('/community/board');" class="btn btn-secondary">
        </span>
      </div>
    </div>
    `;
